...
Observe that $x^2 = (\frac{m}{n})^2 = \frac{m^2}{n^2}$, meaning that $\frac{m^2}{n^2} = 12$.
The facts above imply that 
$$m^2 = 12n^2.$$
We will now proceed with a case split.
Specifically, we will consider two specific cases: the case where $m$ is divisible by 3 and the case in which it is not\footnote{
We can choose whatever case split we want, as long as it benefits us in the proofwriting process.
All we have to do is make sure that we do not accidentally exclude any values of $m$.
Since $m$ either is or is not divisible by 3 for all possible values of $m$, we can safely proceed with our case split.
}.

\vspace{2mm}
\noindent
\underline{$m$ is divisible by 3:}
...
\vspace{2mm}
\noindent
\underline{$m$ is not divisble by 3:}
...
\begin{enumerate}[label=(\alph*)]
    ...
    \begin{align*}
        ...
        &= z &\text{by (M4)}
    \end{align*}
    By the transitivity of =, it follows that $y=z$.
...
\vspace{2mm}
\solve
Suppose $E$ is such a set, and let's give its ordered superset a name, say $S$.
Given that $\alpha$ is a lower bound of $E$, then it must be true that
$$\forall x ~(x \in E \Implies \alpha \leq x).$$
Similarly, since $\beta$ is an upper bound of $E$, then it follows that
$$\forall x~ (x \in E \Implies x \leq \beta).$$

We know that $E$ is nonempty, meaning that there must exist some element $x^* \in E$.
We will use this specific element to prove that $\alpha \leq \beta$.
Recall that, by definition, $a \leq b$ means that either $a < b$ or $a = b$.
So there are four possible cases that can apply to our situation:
\begin{enumerate}[label=(\alph*)]
    \item $\alpha = x^*$ and $x^* = \beta$,
    \item $\alpha = x^*$ and $x^* < \beta$,
    \item $\alpha < x^*$ and $x^* = \beta$, and
    \item $\alpha < x^*$ and $x^* = \beta$.
\end{enumerate}

We will now show that we arrive at the same conclusion in all possible cases—that $\alpha \leq \beta$ in all possible scenarios.

\begin{enumerate}[label=(\alph*)]
    \item \underline{$\alpha = x^*$ and $x^* = \beta$:} In this case, we can simply appeal to the transitivity of $=$ and deduce that $\alpha = \beta$.
    If $\alpha = \beta$, then it clearly follows that $\alpha \leq \beta$.
    \item \underline{$\alpha = x^*$ and $x^* < \beta$:} Here, since $\alpha = x^*$, we can substitute $\alpha$ for $x^*$ in the $x^* < \beta$ inequality.
    This tells us that $\alpha < \beta$, from which it follows that $\alpha \leq \beta$.
    \item \underline{$\alpha < x^*$ and $x^* = \beta$:} Since $\beta = x^*$, we can substitue $\beta$ for $x^*$ in the $\alpha < x^*$ inequality.
    This tells us that $\alpha < \beta$, so it follows that $\alpha \leq \beta$.
    \item \underline{$\alpha < x^*$ and $x^* < \beta$:} In this final case, we can appeal to the transitivity of the $<$ relation.
    After doing this, we get that $\alpha < \beta$, and it once more follows that $\alpha \leq \beta$.
\end{enumerate}

As you can see, no matter which way we go, we arrive at the same conclusion: $\alpha \leq \beta$.
This means that the statement holds in general, and we are done!
$\Box$
...
\vspace{2mm}
\solve
The key to this exercise is to recall that $\real$ has the least-upper-bound property.
This means that both $A$ and $-A$, being subsets of $\real$, must have their infima in $\real$.
Also, as shown in Theorem 1.11 in Rudin, the least-upper-bound property implies the greatest-lower-bound property.
This means that the suprema of $A$ and $-A$ must also exist in $\real$.
Most relevant to the problem at hand, though: 
$$\inf A \in \real \text{ and } \sup (-A) \in \real.$$

Our strategy will be to use the definition of $\inf$ and $\sup$ to show that 
$\inf A \leq -\sup (-A)$ and $\inf A \geq -\sup (-A)$.
Note that if both of these are true at the same time, then $\inf A = -\sup (-A)$, which is what we want.

\vspace{2mm}
\noindent\underline{$\inf A \leq -\sup (-A)$:}
We will now show that $-\sup (-A)$ is a lower bound of $A$.
By definition, $\sup (-A)$ is such that
$$\forall (-x) \in -A,~ -x \leq \sup(-A).$$
Equivalently,
$$\forall (-x) \in -A,~ -(-x) \geq -\sup(-A).$$
We've shown previously that $-(-x) = x$, and we also know that $x \in A$;
this is how the set $-A$ was defined, after all.
So it must be true that
$$\forall x \in A,~ -\sup (-A) \leq -x.$$
In other words, $-\sup (-A)$ is a lower bound of $A$.
By the definition of $\inf A$, we must have that
$$\inf A \leq -\sup(-A).$$


\vspace{2mm}
\noindent
\underline{$\inf A \geq -\sup (-A)$:}
Now we proceed to show that $-\inf A$ is an upper bound of $-A$.
By definition, $\inf A$ is such that 
$$\forall x \in A,~ \inf A \leq x.$$
Observe that this is equivalent to saying that
$$\forall x \in A, ~ -\inf A \geq -x.$$
In other words, 
$$\forall (-x) \in -A, ~ -\inf A \geq -x.$$
This means that $-\inf A$ is an upper bound of $-A$.
By the definition of $\sup (-A)$, it must be true that
$$-\inf A \leq \sup (-A).$$
...
\vspace{2mm}
\noindent
\underline{Conclusion:} The only time in which the statements $\inf A \leq -\sup (-A)$ and $\inf A \geq -\sup (-A)$ can both be true is when $\inf A = -\sup (-A)$, as desired.
$\Box$